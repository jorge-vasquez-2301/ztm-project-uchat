...
pub fn DisplayNameInput(cx: Scope, page_state: UseRef<PageState>) -> Element {
    use uchat_domain::DisplayName;

    let char_count = || page_state.read().display_name.len();
    const MAX_CHARS: usize = DisplayName::MAX_CHARS;
...
pub fn EditProfile(cx: Scope) -> Element {
    ...
    let _fetch_profile = {
        ...
        use_future(cx, (), |_| async move {
            use uchat_endpoint::user::{GetMyProfile, GetMyProfileOk};
            toaster
                .write()
    ...
    let form_onsubmit = async_handler!(
        ...
        move |_| async move {
            use uchat_endpoint::user::{UpdateProfile, UpdateProfileOk};
