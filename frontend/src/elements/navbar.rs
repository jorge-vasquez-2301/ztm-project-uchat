...
pub fn NewPostPopup(cx: Scope, hide: UseState<bool>) -> Element {
    ...
    const BUTTON_CLASS: &str = "
        grid grid-cols-[20px_1fr] gap-4 pl-4
        justify-center items-center
        w-full h-12
        border-y nav-border-color
    ";
...
pub fn NavButton<'a>(cx: Scope<'a, NavButtonProps<'a>>) -> Element<'a> {
...
pub fn NavBar(cx: Scope) -> Element {
    ...
    cx.render(rsx! {
        nav {
            class: "
                max-w-[var(--content-max-width)] h-[var(--navbar-height)]
