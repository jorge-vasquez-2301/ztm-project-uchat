    ...
    pub mod util {
        ...
        pub async fn new_state() -> AppState {
            let connection_url = dotenvy::var("TEST_DATABASE_URL")
                .expect("TEST_DATABASE_URL must be set in order to rn tests");
        ...
        pub async fn api_request_with_router<P: Serialize>(
            router: Router,
            uri: &str,
            payload: P,
        ) -> Response {
        ...
        pub async fn api_request<P: Serialize>(uri: &str, payload: P) -> Response {
